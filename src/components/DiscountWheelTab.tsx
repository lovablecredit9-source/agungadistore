...
export default function DiscountWheelTab() {
  ...
  return (
    <div className="space-y-4 animate-fade-in pb-[calc(9rem+env(safe-area-inset-bottom))]">
      ...
      {/* UPGRADE BATAS PEMBELIAN 30/30 */}
      <div className="rounded-2xl p-4 bg-gradient-to-br from-violet-500/15 via-background to-fuchsia-500/5 border border-violet-400/30">
        <div className="flex items-center gap-1.5 mb-2">
          <Sparkles className="w-4 h-4 text-violet-300" />
          <h2 className="text-sm font-black text-violet-200">Batas Pembelian per Diskon</h2>
        </div>

        {data?.upgradeTier === "permanent" ? (
          <div className="rounded-xl p-3 bg-emerald-500/15 border border-emerald-400/40 flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5 text-emerald-300" />
            <div>
              <div className="text-sm font-black text-emerald-200">Aktif 30/30 — PERMANEN</div>
              <div className="text-[11px] text-emerald-300/80">Selamanya bisa beli 30 hadiah per diskon.</div>
            </div>
          </div>
        ) : data?.upgradeTier === "month" ? (
          <>
            <div className="rounded-xl p-3 bg-cyan-500/15 border border-cyan-400/40 flex items-center gap-2 mb-2.5">
              <CheckCircle2 className="w-5 h-5 text-cyan-300" />
              <div>
                <div className="text-sm font-black text-cyan-200">Aktif 30/30 — Bulanan</div>
                <div className="text-[11px] text-cyan-300/80">
                  Berlaku sampai{" "}
                  {data.upgradeExpiresAt
                    ? new Date(data.upgradeExpiresAt).toLocaleString("id-ID", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })
                    : "-"}
                </div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <Button
                ...
                onClick={() => handleBuyUpgrade("month")}
                className="h-9 text-[11px] font-bold rounded-xl bg-cyan-500/20 border border-cyan-400/40 text-cyan-100 hover:bg-cyan-500/30"
              >
                {upgrading === "month" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <>+30 hari · {data.upgradeMonthCost}<Gem className="w-3 h-3 ml-0.5" /></>}
              </Button>
              <Button
                size="sm"
                disabled={upgrading !== null}
                onClick={() => handleBuyUpgrade("permanent")}
                className="h-9 text-[11px] font-bold rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white"
              >
                {upgrading === "permanent" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <>Permanen · {data.upgradePermanentCost}<Gem className="w-3 h-3 ml-0.5" /></>}
              </Button>
            </div>
          </>
        ) : (
          <>
            <div className="rounded-xl p-3 bg-muted/20 border border-border flex items-center gap-2 mb-2.5">
              <Lock className="w-4 h-4 text-muted-foreground" />
              <div>
                <div className="text-sm font-black text-foreground/80">Gratis — Batas 10/10</div>
                <div className="text-[11px] text-muted-foreground">Upgrade ke 30/30 untuk beli lebih banyak per diskon.</div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <Button
                ...
                disabled={upgrading !== null}
                onClick={() => handleBuyUpgrade("month")}
                className="h-10 text-[11px] font-bold rounded-xl bg-cyan-500/20 border border-cyan-400/40 text-cyan-100 hover:bg-cyan-500/30 flex flex-col leading-tight"
              >
                {upgrading === "month" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : (
                  <>
                    <span>30/30 · 1 Bulan</span>
                    <span className="flex items-center gap-0.5 text-cyan-200">{data?.upgradeMonthCost ?? 250}<Gem className="w-3 h-3" /></span>
                  </>
                )}
              </Button>
              <Button
                ...
                disabled={upgrading !== null}
                onClick={() => handleBuyUpgrade("permanent")}
                className="h-10 text-[11px] font-bold rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white flex flex-col leading-tight"
              >
                {upgrading === "permanent" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : (
                  <>
                    <span>30/30 · Permanen</span>
                    <span className="flex items-center gap-0.5">{data?.upgradePermanentCost ?? 500}<Gem className="w-3 h-3" /></span>
                  </>
                )}
              </Button>
            </div>

...
export default function DiamondRoyale() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950/40 to-black text-white relative overflow-x-hidden">
      {/* bg fx */}
      ...
      {/* Header */}
      <div className="sticky top-0 z-30 backdrop-blur-xl bg-black/60 border-b border-amber-500/20">
        <div className="max-w-md mx-auto px-3 py-2.5 flex items-center justify-between gap-2">
          <button onClick={() => nav("/")} className="p-2 rounded-lg bg-white/5 hover:bg-white/10">
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div className="flex items-center gap-1.5">
            <Diamond className="w-4 h-4 text-amber-300" fill="currentColor" />
            <span className="text-sm font-black bg-gradient-to-r from-amber-300 via-yellow-200 to-amber-400 bg-clip-text text-transparent tracking-wide">
              DIAMOND ROYALE
            </span>
          </div>
      ...
      <div className="max-w-md mx-auto px-3 py-4 space-y-4 relative">
        ...
        <Card className="relative overflow-hidden p-4 bg-gradient-to-br from-amber-500/10 via-purple-500/10 to-pink-500/10 border-amber-400/30">
          ...
          <div className="relative text-center mb-3">
            <Badge className="bg-gradient-to-r from-amber-500 to-orange-500 text-black font-black border-0 mb-2">
              💎 PREMIUM SPIN
            </Badge>
            <h1 className="text-2xl font-black bg-gradient-to-r from-amber-200 via-yellow-300 to-amber-400 bg-clip-text text-transparent">
              Diamond Royale
            </h1>
            <p className="text-[11px] text-white/70 mt-1">Hadiah eksklusif • Pity {pity.hard} spin = legendary</p>
          </div>

          {/* Reel area */}
          <div className="relative bg-black/40 rounded-xl p-3 border border-amber-400/20 min-h-[180px]">
            <AnimatePresence mode="wait">
              {reelSpinning ? (
                <motion.div key="spinning" className="grid grid-cols-5 gap-2"
                ...
                <motion.div key="results"
                  ...
                  className={results.length === 10 ? "grid grid-cols-5 gap-2" : "flex justify-center"}>
                  {results.map((r, i) => {
                    ...
                    return (
                      <motion.div key={i}
                        ...
                        className={`relative ${results.length === 1 ? "w-32 h-32" : "aspect-square"} rounded-xl bg-gradient-to-br ${rs.gradient} ring-2 ${rs.ring} ${rs.glow} shadow-xl flex flex-col items-center justify-center p-1.5`}>
                        <div className={`${results.length === 1 ? "w-10 h-10" : "w-6 h-6"} ${rs.text} mb-0.5`}>
                          {kindIcon(r.kind)}
                        </div>
                ...
                <motion.div key="idle" className="flex flex-col items-center justify-center h-[160px] text-center"
                  initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  <motion.div animate={{ rotate: [0, 10, -10, 0], scale: [1, 1.1, 1] }}
                    transition={{ duration: 3, repeat: Infinity }}>
                    <Diamond className="w-16 h-16 text-amber-300 drop-shadow-[0_0_20px_rgba(251,191,36,0.6)]" fill="currentColor" />
                  </motion.div>
          ...
          <div className="mt-3 space-y-1.5">
            ...
            <p className="text-[10px] text-white/60">
              {pityRemaining <= 10
                ? <span className="text-amber-300 font-black animate-pulse">🔥 {pityRemaining} spin lagi = LEGENDARY guaranteed!</span>
        ...
        <div className="grid grid-cols-3 gap-2">
          <Card className="p-2.5 bg-white/5 border-white/10 text-center">
          ...
          <Card className="p-2.5 bg-white/5 border-amber-400/20 text-center">
          ...
          <Card className="p-2.5 bg-white/5 border-fuchsia-400/20 text-center">
        ...
        <Card className="p-3 bg-white/5 border-white/10">
          <h3 className="text-xs font-black mb-2 flex items-center gap-1.5">
            <Award className="w-4 h-4 text-amber-400" /> Hadiah Pool
          </h3>
            ...
            {prizes.map((p, i) => {
              ...
              return (
                <div key={i} className={`flex items-center justify-between p-2 rounded-lg bg-gradient-to-r ${rs.gradient} bg-opacity-20 border ${rs.ring} ring-1`}>
                  ...
                  <span className="text-[10px] font-bold text-white/70">{((p.weight / prizes.reduce((s, x) => s + x.weight, 0)) * 100).toFixed(1)}%</span>
                </div>
              );
            })}
          </div>

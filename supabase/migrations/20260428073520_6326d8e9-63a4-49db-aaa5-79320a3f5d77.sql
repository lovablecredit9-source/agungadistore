...
DECLARE
  ...
  v_replied bigint := 0;
BEGIN
  -- Product chats: hitung thread yang ada pesan dari user (sender_type='user') dan yang dibalas admin
  WITH pc AS (
    SELECT c.id,
      EXISTS(SELECT 1 FROM public.product_chat_messages m WHERE m.chat_id = c.id AND m.sender_type = 'user') AS has_user,
      EXISTS(SELECT 1 FROM public.product_chat_messages m WHERE m.chat_id = c.id AND m.sender_type = 'admin') AS has_admin
    FROM public.product_chats c
  )
  SELECT COUNT(*) FILTER (WHERE has_user),
         COUNT(*) FILTER (WHERE has_user AND has_admin)

ALTER TABLE public.store_premium_subscriptions
  ADD COLUMN IF NOT EXISTS locked_until timestamptz,
  ADD COLUMN IF NOT EXISTS lock_reason text;

DROP FUNCTION IF EXISTS public.get_store_premium_info(text);
...
BEGIN
  ...
  ELSIF v_sub.locked_until IS NOT NULL AND v_sub.locked_until > now() THEN
    -- Terkunci akibat pelanggaran: manfaat premium dinonaktifkan sementara
    RETURN QUERY SELECT
      false,
      ...
      true,
      v_sub.lock_reason::TEXT,
      v_sub.locked_until::TIMESTAMPTZ;
  ELSE
    RETURN QUERY SELECT
      true,
      v_sub.plan_name::TEXT,
